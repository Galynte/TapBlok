...
1. Download the latest `TapBlok-X.Y.Z.aab` (and optionally the matching `*-native-debug-symbols.zip`) from a GitHub release, or build them yourself with `./gradlew bundleRelease`.
...
The GitHub Actions workflow automatically builds + attaches the AAB and the native symbols zip on every tagged release.