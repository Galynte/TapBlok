...
fun BlockingScreen(
    ...
    // CRITICAL: Default must be false to match Settings default
    val enableBreaks = prefs.getBoolean("enable_breaks", false)
    val currentBreaksRemaining = prefs.getInt("breaks_remaining", 3)

    LaunchedEffect(Unit) {
        breaksRemaining = currentBreaksRemaining
    ...
    Surface(
        ...
        Column(
            ...
            // Only show if explicitly enabled AND breaks remain
            if (enableBreaks && breaksRemaining > 0) {
